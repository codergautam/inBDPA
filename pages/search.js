...
export default function SearchPage({ user, query = "" }) {
  ...
  const [searchResults, setSearchResults] = useState({
    error: "Search for users and opportunities!",
  });
  ...
  return (
    <div className="text-black bg-black flex flex-col min-h-screen w-full">
      <Head>
        <title>inBDPA - Home</title>
        <link rel="icon" href="/favicon.png" />
      </Head>
      <Navbar user={user} />
      <section className="text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 w-full flex-grow">
        <div className="container px-5 py-16 mx-auto">
          ...
          <div className="mt-6 flex flex-col gap-8 items-center justify-center">
            ...
            <div className="text-center m-2">
              {!searchResults.error && <h1>Users</h1>}
            ...
            {!searchResults.error
              ? searchResults.users
                  ...
                  .map((result, index) => {
                    ...
                    if (match && ["about", "username"].includes(match.field)) {
                      ...
                      matched = fieldText.substring(
                        position,
                        position + searchResults.queryLength
                      );
                      afterMatch = fieldText.substring(
                        position + searchResults.queryLength
                      );
                      ...
                      if (beforeMatch.length > 40) {
                        ...
                        beforeMatch = beforeMatch.substring(
                          beforeMatch.length - 40,
                          beforeMatch.length
                        );
                        // add ...
                    ...
                    return (
                      <Link
                        href={`/profile/${result.link}`}
                        key={index}
                        className="flex flex-col bg-gray-200 dark:bg-gray-700 border-b-2 border-gray-300 dark:border-gray-800 w-2/3 mx-auto p-4 rounded-lg overflow-hidden hover:bg-gray-300 dark:hover:bg-gray-600"
                      >
                        <div className="flex flex-row items-center space-x-4 mb-4">
                          ...
                          <div>
                            <h2 className="text-xl font-semibold">
                              ...
                              ) : result.username.length > 80 ? (
                                result.username.substring(0, 80) + "..."
                              ) : (
                                result.username
                              )}
                            </h2>
                            ...
                            <p className="text-gray-600 dark:text-gray-400">
                              {match && match.field === "about" ? (
                                ...
                                result.sections.about.length > 80 ? (
                                result.sections.about.substring(0, 80) + "..."
                              ) : (
                                result?.sections?.about
                              )}
                            </p>
              ...
              : null}
            {!searchResults.error &&
              searchResults.users.length > displayedUsers && (
          ...
          <div className="mt-6 flex flex-col gap-8 items-center justify-center">
            <div className="text-center m-2">
              {!searchResults.error && <h1>Opportunities</h1>}
              {!searchResults.error &&
                searchResults.opportunities.length === 0 && (
            ...
            {!searchResults.error ? (
              searchResults.opportunities
                .slice(0, displayedOpportunities)
                .map((result, index) => {
                  const match = result.match;
                  let beforeMatch = "";
                  let matched = "";
                  ...
                  if (match && ["title", "content"].includes(match.field)) {
                    const position = match.position;
                    const fieldText =
                      match.field === "title" ? result.title : result.content;
            ...
            {!searchResults.error &&
              searchResults.opportunities.length > displayedOpportunities && (
                <div className="text-center">
                  <button
                    className="bg-blue-500 hover:bg-blue-700 text-white font-bold text-lg py-2 px-4 rounded w-full"
                    onClick={() =>
                      setDisplayedOpportunities(displayedOpportunities + 3)
                    }
                  >
