...
// 
// If both conditions are false, it retrieves the article ID from the request body and uses it to get a list of opportunities from the database. It also counts the number of active sessions for each article.
// 
...
import { countSessionsForArticle, getAllOpportunitiesMongo, getOpportunities } from "@/utils/api";
...
export default withIronSessionApiRoute(handler, ironOptions);
 ...
 async function handler(req, res) {
  ...
  // console.log("Received ID: " + article_id)
  // const opportunities = (await getOpportunities(article_id ?? null)).opportunities;
  ...
  const { article_id } = req.body;
  try {
  let opportunities = [...await getAllOpportunitiesMongo(4, article_id ?? null)];
  opportunities = await Promise.all(opportunities.map(async (article) => {
    try {
    const active = (await countSessionsForArticle(article.article_id)).active
    return {...article.toObject(), active}
    } catch(e) {
      console.log(e);
