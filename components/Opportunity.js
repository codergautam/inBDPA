...
//   }, [opportunity.opportunity_id]);
  ...
  return (
    ...
    <Link href={`/opportunity/${opportunity.opportunity_id}`}>
      ...
      <p className="text-lg font-semibold overflow-hidden dark:text-gray-300">{opportunity.title}</p>
      <p className="text-sm ">
        {msToTime(Date.now() - opportunity.createdAt)} ago
      </p>

      <p className="text-sm bg-gray-300 dark:bg-gray-600 dark:text-gray-300 w-fit px-1 rounded-xl">Views: {opportunity.views}</p>
